...
exportAnalyses <- function(outputFolder, exportFolder) {
  ...
  sccsAnalysisListFile <- system.file("settings",
                                      ...
                                      package = "Covid19DrugRepurposing")
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(name, ".json"), package = "Covid19DrugRepurposing")
...
exportMetadata <- function(outputFolder,
  ...
  ParallelLogger::logInfo("- sccs_follow_up_dist table")
  getResult <- function(row) {
    ...
    timesPerPerson <- aggregate(time ~ rowId, sccsEraData$outcomes, sum)
    dist <- quantile(timesPerPerson$time, c(0, 0.1, 0.25, 0.5, 0.85, 0.9, 1))
  ...
  fileName <- file.path(exportFolder, "sccs_follow_up_dist.csv")
...
exportMainResults <- function(outputFolder,
  ...
  analysesSum <- read.csv(file.path(outputFolder, "sccsSummary.csv"))
  ...
  preparePlot <- function(row, reference) {
    ...
    if (file.exists(psFileName)) {
      ...
      if (min(ps$propensityScore) < max(ps$propensityScore)) {
        ps <- SelfControlledCaseSeries:::computePreferenceScore(ps)
    ...
    if (file.exists(psFileName)) {
      ...
      if (is.null(metaData$psError)) {
        cmDataFile <- file.path(outputFolder,
                                "cmOutput",
                                reference$sccsDataFolder[idx][1])
        cmData <- SelfControlledCaseSeries::loadSelfControlledCaseSeriesData(cmDataFile)
        model <- SelfControlledCaseSeries::getPsModel(ps, cmData)
...
prepareKaplanMeier <- function(population) {
  ...
  if (is.null(population$stratumId) || length(unique(population$stratumId)) == nrow(population)/2) {
    ...
    survTarget <- SelfControlledCaseSeries:::adjustedKm(weight = population$weight[idx],
    ...
    survComparator <- SelfControlledCaseSeries:::adjustedKm(weight = population$weight[idx],
                                                            time = population$survivalTime[idx],
                                                            y = population$y[idx])
